...
export const GridScan: React.FC<GridScanProps> = ({
  ...
  // Pointer event listeners (does not capture wheel to preserve smooth scrolling)
  useEffect(() => {
    ...
    if (scanOnClick) el.addEventListener('click', onClick, { passive: true });
  ...
  // Main Three.js WebGL initialization
  useEffect(() => {
    ...
    // Smart IntersectionObserver: automatically pause rendering loop when scrolled offscreen
    ...
    const tick = () => {
      if (!isVisibleRef.current) {
        // Paused while out of view to save 100% GPU cycles
