  ...
  useEffect(() => {
    ...
    function movePoints(time: number) {
      ...
      lines.forEach((pts) => {
        pts.forEach((p) => {
          const move =
            noise.perlin2((p.x + time * speedX) * 0.002, (p.y + time * speedY) * 0.0015) * 12;
          ...
          p.cursor.vx += (0 - p.cursor.x) * ten;
          p.cursor.vy += (0 - p.cursor.y) * ten;
          p.cursor.vx *= f;
          p.cursor.vy *= f;
    ...
    function drawLines() {
      ...
      linesRef.current.forEach((points) => {
        ...
        points.forEach((p, idx) => {
          ...
          p1 = moved(p, !isLast);
          const nextPt = points[idx + 1] || points[points.length - 1];
          const p2 = moved(nextPt, !isLast);
