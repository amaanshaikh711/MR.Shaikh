...
export const Scene3D: React.FC<Scene3DProps> = ({
  ...
  useEffect(() => {
    ...
    if (variant === 'hero') {
      // Architectural multifaceted crystal with dual wireframe shell and orbital rings
      const geom = new THREE.IcosahedronGeometry(1.3, 1);
      const mat = new THREE.MeshPhysicalMaterial({
        color: isDark ? 0x161622 : 0xedebe8,
        roughness: isDark ? 0.2 : 0.35,
        metalness: isDark ? 0.8 : 0.3,
        transmission: isDark ? 0.3 : 0.1,
        reflectivity: 0.9,
        clearcoat: 0.8,
      ...
      group.add(mainMesh);
      animObjects.push({ mesh: mainMesh, rotSpeedX: 0.003, rotSpeedY: 0.005 });

      // Outer wireframe cage
      const wireGeom = new THREE.IcosahedronGeometry(1.34, 1);
      const wireMat = new THREE.MeshBasicMaterial({
        color: isDark ? 0xc084fc : 0x4f46e5,
        wireframe: true,
        transparent: true,
      ...
      group.add(wireMesh);
      animObjects.push({ mesh: wireMesh, rotSpeedX: 0.003, rotSpeedY: 0.005 });
      ...
      for (let i = 0; i <= 64; i++) {
        ...
        ringPts1.push(new THREE.Vector3(Math.cos(theta) * 2.1, Math.sin(theta) * 2.1, 0));
      }
      ringGeom1.setFromPoints(ringPts1);
      ...
      group.add(ring1);
      animObjects.push({ mesh: ring1, rotSpeedZ: 0.003 });

      const ringGeom2 = new THREE.BufferGeometry();
      const ringPts2 = [];
      for (let i = 0; i <= 64; i++) {
        const theta = (i / 64) * Math.PI * 2;
        ringPts2.push(new THREE.Vector3(Math.cos(theta) * 2.4, 0, Math.sin(theta) * 2.4));
      }
      ringGeom2.setFromPoints(ringPts2);
      ...
      group.add(ring2);
      animObjects.push({ mesh: ring2, rotSpeedY: -0.004 });

      // Stardust Sparkles
      const partCount = 45;
      const partPos = new Float32Array(partCount * 3);
      for (let i = 0; i < partCount * 3; i += 3) {
        partPos[i] = (Math.random() - 0.5) * 5;
        partPos[i + 1] = (Math.random() - 0.5) * 5;
        partPos[i + 2] = (Math.random() - 0.5) * 5;
      }
      ...
      const pointsMesh = new THREE.Points(partGeom, partMat);
      group.add(pointsMesh);
      animObjects.push({ mesh: pointsMesh, rotSpeedY: 0.001 });
